...
class AI_Community_Provider_Dropbox extends AI_Community_OAuth2_Provider {
    ...
    // API endpoint'leri
    protected $auth_url = 'https://www.dropbox.com/oauth2/authorize';
    protected $token_url = 'https://api.dropboxapi.com/oauth2/token';
    protected $userinfo_url = 'https://api.dropboxapi.com/2/users/get_current_account';
    
    // OAuth2 ayarları
    protected $scope = 'account_info.read';
    protected $token_method = 'POST';
    protected $userinfo_method = 'POST'; // Dropbox POST kullanır
    ...
    public function handle() {
        ...
        if (isset($params['error'])) {
            ...
            $this->core->log('dropbox_oauth_error', [
                'error' => $error,
    ...
    protected function handle_callback($params) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_state')) {
            $this->core->log('dropbox_invalid_state', ['state' => $state]);
        ...
        if (is_wp_error($token_data)) {
            $this->core->log('dropbox_token_error', [
        ...
        if (is_wp_error($userinfo)) {
            $this->core->log('dropbox_userinfo_error', [
        ...
        if (!$user_id || is_wp_error($user_id)) {
            $this->core->log('dropbox_user_creation_error', [
        ...
        // Başarılı giriş
        $redirect_url = apply_filters('ai_community_login_redirect', home_url('/dashboard'), $user_id, 'dropbox');
    ...
    public function parse_expires_in($response) {
        return $response['expires_in'] ?? 14400;
    }
    ...
    public function get_userinfo_params($access_token) {
        return json_encode(['include_mounted_folders' => true]);
    }
    ...
    public function get_userinfo($access_token) {
        ...
        if (isset($data['error'])) {
            return new WP_Error('dropbox_api_error', $data['error_summary'] ?? 'Unknown error');
        }
    ...
    public function get_access_token($code) {
        ...
        if (isset($data['error'])) {
            return new WP_Error('dropbox_token_error', $data['error_description'] ?? $data['error'] ?? 'Unknown error');
        }
    ...
    public function map_userinfo_to_profile($userinfo) {
        if (!is_array($userinfo) || empty($userinfo['account_id'])) {
            return [
                'id' => '',
                'email' => '',
                'name' => '',
        ...
        // İsim ve soyisim
        $first_name = $userinfo['name']['given_name'] ?? '';
        $last_name = $userinfo['name']['surname'] ?? '';
        ...
        return [
            'id' => (string)($userinfo['account_id'] ?? ''),
            'email' => $userinfo['email'] ?? '',
            'email_verified' => $userinfo['email_verified'] ?? false,
            'name' => $display_name,
            'first_name' => $first_name,
            'last_name' => $last_name,
            'username' => $userinfo['email'] ? explode('@', $userinfo['email'])[0] : '',
            'avatar' => '', // Dropbox profil fotoğrafı vermiyor
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['country'])) {
            update_user_meta($user_id, 'dropbox_country', $profile['country']);
        }
        
        if (!empty($profile['locale'])) {
            update_user_meta($user_id, 'dropbox_locale', $profile['locale']);
        }
    ...
    protected function generate_unique_username($profile) {
        ...
        if (!empty($profile['username'])) {
            ...
            $base = 'dropbox_' . substr($profile['id'], 0, 8);
        } else {
            $base = 'dropbox_user';
        }

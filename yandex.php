<?php
/**
 * AI Community Provider: Yandex
 * Rusya'nın en büyük arama motoru ve teknoloji şirketi
 * 
...
/**
 * Yandex OAuth Provider sınıfı
 */
class AI_Community_Provider_Yandex extends AI_Community_OAuth2_Provider {
     ...
     * @var string
     */
    protected $provider_name = 'Yandex';
    ...
    /**
     * İzin kapsamı (scope)
     * Yandex scopes: https://yandex.com/dev/oauth/doc/dg/reference/console-client.html
     *
     * @var string
     */
    protected $scope = 'login:email login:info login:avatar';
    ...
    public function __construct($id, $config = [], $core = null) {
        ...
        if (isset($config['redirect_uri'])) {
            ...
            $this->redirect_uri = home_url('/ai-community/?provider=yandex&action=callback');
        }
    ...
    public function get_auth_url($state = '') {
        ...
        if (!empty($state)) {
            ...
            $params['state'] = wp_create_nonce('yandex_oauth_' . get_current_user_id());
        }
     ...
     * Yandex API yanıtı: https://yandex.com/dev/passport/doc/dg/reference/response.html
     *
    ...
    public function map_userinfo_to_profile($userinfo) {
        ...
        // Email
        $profile['email'] = isset($userinfo['default_email']) ? $userinfo['default_email'] : '';
        if (empty($profile['email']) && isset($userinfo['emails'][0])) {
            $profile['email'] = $userinfo['emails'][0];
        }
        
        // İsim
        if (isset($userinfo['real_name']) && !empty($userinfo['real_name'])) {
            $profile['name'] = $userinfo['real_name'];
        } elseif (isset($userinfo['display_name']) && !empty($userinfo['display_name'])) {
            $profile['name'] = $userinfo['display_name'];
            ...
            $profile['name'] = 'Yandex User';
        }
        
        // Avatar - Yandex avatar URL'i oluştur
        if (isset($userinfo['default_avatar_id'])) {
            $profile['avatar'] = 'https://avatars.yandex.net/get-yapic/' . $userinfo['default_avatar_id'] . '/islands-200';
        } else {
            $profile['avatar'] = '';
        }
        ...
        if (isset($userinfo['sex'])) {
            $profile['gender'] = ($userinfo['sex'] == 'male') ? 'male' : 'female';
        }
        
        if (isset($userinfo['default_phone']['number'])) {
            $profile['phone'] = $userinfo['default_phone']['number'];
        }
    ...
    protected function handle_callback($request) {
        ...
        if (!empty($state)) {
            if (!wp_verify_nonce($state, 'yandex_oauth_' . get_current_user_id())) {
                return [
                    'success' => false,
    ...
    protected function handle_auth($request) {
        $state = wp_create_nonce('yandex_oauth_' . get_current_user_id());

<?php
/**
 * Hulu Mock OAuth2 Provider
 * 
 * NOT: Hulu gerçek OAuth2 API'si sunmamaktadır.
...
class AI_Community_Provider_Hulu extends AI_Community_OAuth2_Provider {
    
    protected $label = 'Hulu (Mock)';
    protected $name = 'Hulu';
    public $slug = 'hulu';
    protected $is_mock = true;
    
    // Mock API endpoint'leri
    protected $auth_url = 'https://mock-auth.local/hulu';
    protected $token_url = 'https://mock-token.local/hulu';
    protected $userinfo_url = 'https://mock-api.local/hulu/user';
    ...
    public function __construct($id, $config, $core) {
        ...
        $this->core->log('hulu_mock_mode', [
            'message' => 'Hulu provider MOCK modunda çalışıyor - Gerçek API yok'
        ]);
    }
    ...
    public function handle() {
        ...
        if (isset($params['error'])) {
            ...
            $this->core->log('hulu_oauth_error', [
                'error' => $error,
    ...
    protected function handle_callback($params) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_state')) {
            $this->core->log('hulu_invalid_state', ['state' => $state]);
        ...
        if (is_wp_error($token_data)) {
            $this->core->log('hulu_token_error', [
        ...
        if (is_wp_error($userinfo)) {
            $this->core->log('hulu_userinfo_error', [
        ...
        if (!$user_id || is_wp_error($user_id)) {
            $this->core->log('hulu_user_creation_error', [
        ...
        // Başarılı giriş
        $redirect_url = apply_filters('ai_community_login_redirect', home_url('/dashboard'), $user_id, 'hulu');
    ...
    public function get_status() {
        return [
            ...
            'warning' => 'Hulu gerçek API sunmamaktadır. Bu provider sadece test amaçlıdır.'
        ];
    }
    ...
    public function get_userinfo($access_token) {
        // Mock kullanıcı verileri - Hulu temalı
        $mock_users = [
            [
                'id' => 'hulu_usr_001',
                ...
                'given_name' => 'Sarah',
                'family_name' => 'Watson',
                'picture' => 'https://ui-avatars.com/api/?name=Sarah+Watson&background=1db954&color=fff&size=200',
                'subscription' => 'hulu_no_ads',
                'member_since' => '2020-02-15',
                'profiles' => 4,
                'locale' => 'en-US',
                'favorite_show' => 'The Handmaid\'s Tale',
                'watch_time' => '1500 hours'
            ],
            [
                'id' => 'hulu_usr_002',
                ...
                'given_name' => 'Mike',
                'family_name' => 'Chen',
                'picture' => 'https://ui-avatars.com/api/?name=Mike+Chen&background=1db954&color=fff&size=200',
                'subscription' => 'hulu_with_ads',
                'member_since' => '2021-05-20',
                'profiles' => 3,
                'locale' => 'en-US',
                'favorite_show' => 'Only Murders in the Building',
                'watch_time' => '850 hours'
            ],
            [
                'id' => 'hulu_usr_003',
                ...
                'given_name' => 'Emily',
                'family_name' => 'Davis',
                'picture' => 'https://ui-avatars.com/api/?name=Emily+Davis&background=1db954&color=fff&size=200',
                'subscription' => 'hulu_live_tv',
                'member_since' => '2022-01-10',
                'profiles' => 5,
                'locale' => 'en-US',
                'favorite_show' => 'Grey\'s Anatomy',
                'watch_time' => '2100 hours'
            ],
            [
                'id' => 'hulu_usr_004',
                'email' => 'ayse.demir@example.com.tr',
                'name' => 'Ayşe Demir',
                'given_name' => 'Ayşe',
                'family_name' => 'Demir',
                'picture' => 'https://ui-avatars.com/api/?name=Ayse+Demir&background=1db954&color=fff&size=200',
                'subscription' => 'hulu_no_ads',
                'member_since' => '2023-03-15',
                'profiles' => 2,
                'locale' => 'tr-TR',
                'favorite_show' => 'The Bear',
                'watch_time' => '350 hours'
            ]
        ];
    ...
    public function get_access_token($code) {
        return [
            'access_token' => 'mock_at_hulu_' . wp_generate_password(24, false),
            'refresh_token' => 'mock_rt_hulu_' . wp_generate_password(24, false),
            'expires_in' => 3600,
            'token_type' => 'Bearer',
            'scope' => $this->scope
        ];
    ...
    public function map_userinfo_to_profile($userinfo) {
        if (!is_array($userinfo) || empty($userinfo['id'])) {
            return [
                'id' => 'mock_' . uniqid(),
                'email' => 'mock_' . uniqid() . '@hulu.local',
                'name' => 'Hulu User',
                'avatar' => 'https://ui-avatars.com/api/?name=Hulu+User&background=1db954&color=fff&size=200'
            ];
        }
        ...
        return [
            ...
            'email_verified' => true,
            'name' => $userinfo['name'] ?? 'Hulu User',
            'first_name' => $userinfo['given_name'] ?? '',
            'last_name' => $userinfo['family_name'] ?? '',
            'username' => explode('@', $userinfo['email'])[0] ?? 'hulu_user',
            'avatar' => $userinfo['picture'] ?? '',
            'subscription' => $userinfo['subscription'] ?? 'hulu_with_ads',
            'member_since' => $userinfo['member_since'] ?? date('Y-m-d'),
            'profiles' => $userinfo['profiles'] ?? 3,
            'locale' => $userinfo['locale'] ?? 'en-US',
            'favorite_show' => $userinfo['favorite_show'] ?? 'Various',
            'watch_time' => $userinfo['watch_time'] ?? '0 hours'
        ];
    }
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['subscription'])) {
            update_user_meta($user_id, 'hulu_subscription', $profile['subscription']);
        }
        
        if (!empty($profile['favorite_show'])) {
            update_user_meta($user_id, 'hulu_favorite_show', $profile['favorite_show']);
        }
        
        if (!empty($profile['watch_time'])) {
            update_user_meta($user_id, 'hulu_watch_time', $profile['watch_time']);
        }
    ...
    protected function generate_unique_username($profile) {
        ...
        if (!empty($profile['username'])) {
            ...
            $base = 'hulu_' . substr($profile['id'], -8);
        } else {
            $base = 'hulu_user';
        }

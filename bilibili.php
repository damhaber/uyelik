<?php
// masal-panel/modules/ai-community-engine/core/providers/bilibili.php
// Bilibili (B站) Provider - TÜM ABSTRACT METODLAR IMPLEMENTE EDİLDİ
...
class AI_Community_Provider_Bilibili extends AI_Community_OAuth2_Provider {
    
    protected $label = 'Bilibili (B站)';
    ...
    public function get_auth_url($state = '') {
        $url = 'https://api.bilibili.com/x/account/oauth2/authorize';
        $params = [
    ...
    public function get_token_url() {
        return 'https://api.bilibili.com/x/account/oauth2/token';
    }
    ...
    public function get_userinfo_url() {
        return 'https://api.bilibili.com/x/account/info';
    }
    ...
    public function handle() {
        if (!$this->is_active()) {
            $this->core->log('bilibili_inactive', ['provider' => $this->id]);
    ...
    /**
     * Bilibili özel callback handler - PARAMETRELİ ($request)
     */
    protected function handle_callback($request) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_' . $this->id)) {
            $this->core->log('bilibili_state_invalid', ['provider' => $this->id]);
        ...
        if (is_wp_error($token)) {
            $this->core->log('bilibili_token_error', [
                'provider' => $this->id,
        ...
        if (is_wp_error($user_profile)) {
            $this->core->log('bilibili_userinfo_error', [
                'provider' => $this->id,
        ...
        if (is_wp_error($wp_user_id) || !$wp_user_id) {
            $this->core->log('bilibili_wp_user_error', [
                'provider' => $this->id,
        ...
        $this->core->log('bilibili_login_success', [
            'provider' => $this->id,
            'user_id'  => $wp_user_id,
        ]);
    ...
    protected function start_auth() {
        ...
        $this->core->log('bilibili_auth_start', [
            'provider' => $this->id,
            'auth_url' => $auth_url
        ]);
    ...
    public function map_userinfo_to_profile($body) {
        ...
        if (!is_array($data) || !isset($data['code']) || $data['code'] !== 0) {
            $msg = isset($data['message']) ? $data['message'] : 'Geçersiz Bilibili kullanıcı bilgisi';
            return new WP_Error('bilibili_userinfo_invalid', $msg);
        }
        
        $user_data = isset($data['data']) ? $data['data'] : [];
        
        return [
            'id'      => isset($user_data['mid']) ? (string)$user_data['mid'] : '',
            'email'   => isset($user_data['email']) ? $user_data['email'] : '',
            'name'    => isset($user_data['uname']) ? $user_data['uname'] : 'Bilibili User',
            'avatar'  => isset($user_data['face']) ? $user_data['face'] : '',
            'gender'  => isset($user_data['gender']) ? $this->normalize_gender($user_data['gender']) : 'other',
            'level'   => isset($user_data['level']) ? $user_data['level'] : 0,
            'vip'     => isset($user_data['vip_type']) ? ($user_data['vip_type'] > 0) : false,
        ];
    }
    ...
    public function find_or_create_user($profile) {
        ...
        $email = $profile['email'] ?: $username . '@bilibili.user';

<?php
// masal-panel/modules/ai-community-engine/core/providers/zhihu.php
// Zhihu (知乎) Provider - TÜM HATALAR DÜZELTİLDİ
...
class AI_Community_Provider_Zhihu extends AI_Community_OAuth2_Provider {
    
    protected $label = 'Zhihu (知乎)';
    ...
    public function get_auth_url($state = '') {
        $url = 'https://www.zhihu.com/oauth/authorize';
        $params = [
    ...
    public function get_token_url() {
        return 'https://www.zhihu.com/oauth/access_token';
    }
    ...
    public function handle() {
        if (!$this->is_active()) {
            $this->core->log('zhihu_inactive', ['provider' => $this->id]);
    ...
    /**
     * Zhihu özel callback handler - PARAMETRELİ ($request)
     */
    protected function handle_callback($request) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_' . $this->id)) {
            $this->core->log('zhihu_state_invalid', ['provider' => $this->id]);
        ...
        if (is_wp_error($token)) {
            $this->core->log('zhihu_token_error', [
                'provider' => $this->id,
        ...
        if (is_wp_error($user_profile)) {
            $this->core->log('zhihu_userinfo_error', [
                'provider' => $this->id,
        ...
        if (is_wp_error($wp_user_id) || !$wp_user_id) {
            $this->core->log('zhihu_wp_user_error', [
                'provider' => $this->id,
        ...
        $this->core->log('zhihu_login_success', [
            'provider' => $this->id,
            'user_id'  => $wp_user_id,
        ]);
    ...
    protected function start_auth() {
        ...
        $this->core->log('zhihu_auth_start', [
            'provider' => $this->id,
            'auth_url' => $auth_url
        ]);
    ...
    public function map_userinfo_to_profile($body) {
        ...
        if (!is_array($data) || empty($data['id'])) {
            return new WP_Error('zhihu_userinfo_invalid', 'Geçersiz Zhihu kullanıcı bilgisi');
        }
        
        return [
            'id'        => isset($data['id']) ? (string)$data['id'] : '',
            'email'     => isset($data['email']) ? $data['email'] : '',
            'name'      => isset($data['name']) ? $data['name'] : 'Zhihu User',
            'headline'  => isset($data['headline']) ? $data['headline'] : '',
            'avatar'    => isset($data['avatar_url']) ? $data['avatar_url'] : '',
            ...
            'following_count' => isset($data['following_count']) ? $data['following_count'] : 0,
            'follower_count'  => isset($data['follower_count']) ? $data['follower_count'] : 0,
            'voteup_count'    => isset($data['voteup_count']) ? $data['voteup_count'] : 0,
            'answer_count'    => isset($data['answer_count']) ? $data['answer_count'] : 0,
            'question_count'  => isset($data['question_count']) ? $data['question_count'] : 0,
        ];
    }
    ...
    public function find_or_create_user($profile) {
        ...
        $email = $profile['email'] ?: $username . '@zhihu.user';
    ...
    private function normalize_gender($gender) {
        ...
        if ($gender === 0 || $gender === 'female') return 'female';
        return 'other';
    }
}
<?php
/**
 * HBO Max Mock OAuth2 Provider
 * 
 * NOT: HBO Max gerçek OAuth2 API'si sunmamaktadır.
...
class AI_Community_Provider_HBOMax extends AI_Community_OAuth2_Provider {
    
    protected $label = 'HBO Max (Mock)';
    protected $name = 'HBO Max';
    public $slug = 'hbo-max';
    protected $is_mock = true;
    
    // Mock API endpoint'leri
    protected $auth_url = 'https://mock-auth.local/hbomax';
    protected $token_url = 'https://mock-token.local/hbomax';
    protected $userinfo_url = 'https://mock-api.local/hbomax/user';
    ...
    public function __construct($id, $config, $core) {
        ...
        $this->core->log('hbomax_mock_mode', [
            'message' => 'HBO Max provider MOCK modunda çalışıyor - Gerçek API yok'
        ]);
    }
    ...
    public function handle() {
        ...
        if (isset($params['error'])) {
            ...
            $this->core->log('hbomax_oauth_error', [
                'error' => $error,
    ...
    protected function handle_callback($params) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_state')) {
            $this->core->log('hbomax_invalid_state', ['state' => $state]);
        ...
        if (is_wp_error($token_data)) {
            $this->core->log('hbomax_token_error', [
        ...
        if (is_wp_error($userinfo)) {
            $this->core->log('hbomax_userinfo_error', [
        ...
        if (!$user_id || is_wp_error($user_id)) {
            $this->core->log('hbomax_user_creation_error', [
        ...
        // Başarılı giriş
        $redirect_url = apply_filters('ai_community_login_redirect', home_url('/dashboard'), $user_id, 'hbo-max');
    ...
    public function get_status() {
        return [
            ...
            'warning' => 'HBO Max gerçek API sunmamaktadır. Bu provider sadece test amaçlıdır.'
        ];
    }
    ...
    public function get_userinfo($access_token) {
        // Mock kullanıcı verileri - HBO Max temalı
        $mock_users = [
            [
                'id' => 'hbomax_usr_001',
                ...
                'given_name' => 'Jon',
                'family_name' => 'Snow',
                'picture' => 'https://ui-avatars.com/api/?name=Jon+Snow&background=6b46c1&color=fff&size=200',
                'subscription' => 'ultimate',
                'member_since' => '2020-05-27',
                'profiles' => 5,
                'locale' => 'en-US',
                'favorite_show' => 'Game of Thrones',
                'total_watch_time' => '2500 hours'
            ],
            [
                'id' => 'hbomax_usr_002',
                ...
                'picture' => 'https://ui-avatars.com/api/?name=Daenerys+Targaryen&background=6b46c1&color=fff&size=200',
                'subscription' => 'ultimate',
                'member_since' => '2020-05-27',
                'profiles' => 3,
                'locale' => 'en-US',
                'favorite_show' => 'House of the Dragon',
                'total_watch_time' => '1800 hours'
            ],
            [
                'id' => 'hbomax_usr_003',
                ...
                'given_name' => 'Tony',
                'family_name' => 'Soprano',
                'picture' => 'https://ui-avatars.com/api/?name=Tony+Soprano&background=6b46c1&color=fff&size=200',
                'subscription' => 'standard',
                'member_since' => '2021-09-15',
                'profiles' => 4,
                'locale' => 'en-US',
                'favorite_show' => 'The Sopranos',
                'total_watch_time' => '1200 hours'
            ],
            [
                'id' => 'hbomax_usr_004',
                'email' => 'barry.berkman@example.com',
                'name' => 'Barry Berkman',
                'given_name' => 'Barry',
                'family_name' => 'Berkman',
                'picture' => 'https://ui-avatars.com/api/?name=Barry+Berkman&background=6b46c1&color=fff&size=200',
                'subscription' => 'standard',
                'member_since' => '2022-03-20',
                'profiles' => 2,
                'locale' => 'en-US',
                'favorite_show' => 'Barry',
                'total_watch_time' => '450 hours'
            ],
            [
                'id' => 'hbomax_usr_005',
                'email' => 'mehmet.yilmaz@example.com.tr',
                'name' => 'Mehmet Yılmaz',
                'given_name' => 'Mehmet',
                'family_name' => 'Yılmaz',
                'picture' => 'https://ui-avatars.com/api/?name=Mehmet+Yilmaz&background=6b46c1&color=fff&size=200',
                'subscription' => 'ultimate',
                'member_since' => '2023-01-10',
                'profiles' => 4,
                'locale' => 'tr-TR',
                'favorite_show' => 'Chernobyl',
                'total_watch_time' => '680 hours'
            ]
        ];
    ...
    public function get_access_token($code) {
        return [
            'access_token' => 'mock_at_hbomax_' . wp_generate_password(24, false),
            'refresh_token' => 'mock_rt_hbomax_' . wp_generate_password(24, false),
            'expires_in' => 3600,
            'token_type' => 'Bearer',
            'scope' => $this->scope
        ];
    ...
    public function map_userinfo_to_profile($userinfo) {
        if (!is_array($userinfo) || empty($userinfo['id'])) {
            return [
                'id' => 'mock_' . uniqid(),
                'email' => 'mock_' . uniqid() . '@hbomax.local',
                'name' => 'HBO Max User',
                'avatar' => 'https://ui-avatars.com/api/?name=HBO+Max+User&background=6b46c1&color=fff&size=200'
            ];
        }
        ...
        return [
            ...
            'email_verified' => true,
            'name' => $userinfo['name'] ?? 'HBO Max User',
            'first_name' => $userinfo['given_name'] ?? '',
            'last_name' => $userinfo['family_name'] ?? '',
            'username' => explode('@', $userinfo['email'])[0] ?? 'hbomax_user',
            'avatar' => $userinfo['picture'] ?? '',
            'subscription' => $userinfo['subscription'] ?? 'standard',
            'member_since' => $userinfo['member_since'] ?? date('Y-m-d'),
            'profiles' => $userinfo['profiles'] ?? 4,
            'locale' => $userinfo['locale'] ?? 'en-US',
            'favorite_show' => $userinfo['favorite_show'] ?? 'Game of Thrones',
            'total_watch_time' => $userinfo['total_watch_time'] ?? '0 hours'
        ];
    }
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['subscription'])) {
            update_user_meta($user_id, 'hbomax_subscription', $profile['subscription']);
        }
        
        if (!empty($profile['favorite_show'])) {
            update_user_meta($user_id, 'hbomax_favorite_show', $profile['favorite_show']);
        }
        
        if (!empty($profile['total_watch_time'])) {
            update_user_meta($user_id, 'hbomax_watch_time', $profile['total_watch_time']);
        }
    ...
    protected function generate_unique_username($profile) {
        ...
        if (!empty($profile['username'])) {
            ...
            $base = 'hbomax_' . substr($profile['id'], -8);
        } else {
            $base = 'hbomax_user';
        }

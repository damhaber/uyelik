...
class AI_Community_Provider_Bitbucket extends AI_Community_OAuth2_Provider {
    
    protected $label = 'Bitbucket';
    protected $name = 'Bitbucket';
    public $slug = 'bitbucket';
    
    // API endpoint'leri
    protected $auth_url = 'https://bitbucket.org/site/oauth2/authorize';
    protected $token_url = 'https://bitbucket.org/site/oauth2/access_token';
    protected $userinfo_url = 'https://api.bitbucket.org/2.0/user';
    protected $emails_url = 'https://api.bitbucket.org/2.0/user/emails';
    
    // OAuth2 ayarları
    protected $scope = 'account email';
    ...
    public function __construct($id, $config, $core) {
        ...
        // Bitbucket özel yapılandırmalar
    ...
    public function handle() {
        ...
        if (isset($params['error'])) {
            ...
            $this->core->log('bitbucket_oauth_error', [
                'error' => $error,
    ...
    protected function handle_callback($params) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_state')) {
            $this->core->log('bitbucket_invalid_state', ['state' => $state]);
        ...
        if (is_wp_error($token_data)) {
            $this->core->log('bitbucket_token_error', [
        ...
        if (is_wp_error($userinfo)) {
            $this->core->log('bitbucket_userinfo_error', [
        ...
        // Email bilgilerini al (Bitbucket email'i ayrı endpoint'ten alır)
        $emails = $this->get_user_emails($token_data['access_token']);
        
        // Profil oluştur
        $profile = $this->map_userinfo_to_profile($userinfo, $emails);
        
        // Kullanıcı bul veya oluştur
        $user_id = $this->find_or_create_user($profile);
        
        if (!$user_id || is_wp_error($user_id)) {
            $this->core->log('bitbucket_user_creation_error', [
        ...
        // Başarılı giriş
        $redirect_url = apply_filters('ai_community_login_redirect', home_url('/dashboard'), $user_id, 'bitbucket');
     ...
     * @return array
     */
    protected function get_user_emails($access_token) {
        ...
        if (is_wp_error($response)) {
            $this->core->log('bitbucket_emails_error', [
    ...
    public function get_userinfo($access_token) {
        ...
        if (isset($data['error'])) {
            return new WP_Error('bitbucket_api_error', $data['error']['message'] ?? 'Unknown error');
        }
    ...
    /**
     * Access token al
     * Bitbucket Basic Auth gerektirir
     * 
     * @param string $code Yetkilendirme kodu
     * @return array|WP_Error
     */
    public function get_access_token($code) {
        ...
        if (isset($data['error'])) {
            return new WP_Error('bitbucket_token_error', $data['error_description'] ?? $data['error'] ?? 'Unknown error');
        }
     ...
     * @param array $emails Email verileri
     * @return array
     */
    public function map_userinfo_to_profile($userinfo, $emails = []) {
        if (!is_array($userinfo) || empty($userinfo['uuid'])) {
            return [
                'id' => '',
                'email' => '',
                'name' => '',
        ...
        foreach ($emails as $email_data) {
            if (!empty($email_data['is_primary']) && $email_data['is_primary'] === true) {
        ...
        // Avatar URL
        $avatar = $userinfo['links']['avatar']['href'] ?? '';
        if (!empty($avatar) && strpos($avatar, '?') === false) {
            $avatar .= '?s=200';
        }
        
        return [
            'id' => (string)($userinfo['uuid'] ?? ''),
            'email' => $email,
            'name' => $display_name,
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['location'])) {
            update_user_meta($user_id, 'location', $profile['location']);
        }
    ...
    protected function generate_unique_username($profile) {
        ...
        } elseif (!empty($profile['nickname'])) {
            $base = sanitize_user($profile['nickname'], true);
        } elseif (!empty($profile['id'])) {
            $base = 'bitbucket_' . substr($profile['id'], 0, 8);
        } else {
            $base = 'bitbucket_user';
        }

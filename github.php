...
class AI_Community_Provider_Github extends AI_Community_OAuth2_Provider {

    protected $label = 'GitHub';
    public $slug = 'github';
    protected $scope = 'read:user user:email';
    protected $auth_url = 'https://github.com/login/oauth/authorize';
    protected $token_url = 'https://github.com/login/oauth/access_token';
    protected $userinfo_url = 'https://api.github.com/user';
    protected $emails_url = 'https://api.github.com/user/emails';
    ...
    public function get_auth_url($state = '') {
        ...
        if (!empty($state)) {
            ...
            $params['state'] = wp_create_nonce('github_oauth_state');
        }
    ...
    public function parse_refresh_token($response) {
        // GitHub refresh token kullanmaz
        return null;
    }
    
    public function parse_expires_in($response) {
        // GitHub expires_in döndürmez
        return 3600;
    }
    ...
    public function get_userinfo_headers($token) {
        return [
            ...
            'Accept' => 'application/json',
            'User-Agent' => 'AI-Community-Plugin'
        ];
    }
    ...
    /**
     * GitHub email bilgilerini al (email public değilse)
     */
    public function get_user_emails($token) {
        ...
        $response = wp_remote_get($this->emails_url, [
            'headers' => $headers,
            'timeout' => 30
        ]);
        
        if (is_wp_error($response)) {
            error_log('GitHub emails alma hatası: ' . $response->get_error_message());
            return [];
        }
    ...
    public function map_userinfo_to_profile($userinfo) {
        ...
        $profile = [
            ...
            'username' => isset($userinfo['login']) ? $userinfo['login'] : '',
            'display_name' => isset($userinfo['name']) ? $userinfo['name'] : '',
            'email' => isset($userinfo['email']) ? $userinfo['email'] : '',
            'avatar' => isset($userinfo['avatar_url']) ? $userinfo['avatar_url'] : '',
            'profile_url' => isset($userinfo['html_url']) ? $userinfo['html_url'] : '',
            'bio' => isset($userinfo['bio']) ? $userinfo['bio'] : '',
            'company' => isset($userinfo['company']) ? $userinfo['company'] : '',
            'location' => isset($userinfo['location']) ? $userinfo['location'] : '',
            'blog' => isset($userinfo['blog']) ? $userinfo['blog'] : '',
            'hireable' => isset($userinfo['hireable']) ? (bool) $userinfo['hireable'] : false,
            'public_repos' => isset($userinfo['public_repos']) ? (int) $userinfo['public_repos'] : 0,
            'public_gists' => isset($userinfo['public_gists']) ? (int) $userinfo['public_gists'] : 0,
            'followers' => isset($userinfo['followers']) ? (int) $userinfo['followers'] : 0,
            'following' => isset($userinfo['following']) ? (int) $userinfo['following'] : 0,
            'created_at' => isset($userinfo['created_at']) ? $userinfo['created_at'] : '',
            'updated_at' => isset($userinfo['updated_at']) ? $userinfo['updated_at'] : '',
            'verified' => true
        ];
    ...
    public function handle_callback($request) {
        try {
            // State kontrolü
            if (!isset($request['state']) || !wp_verify_nonce($request['state'], 'github_oauth_state')) {
            ...
            if (isset($request['error'])) {
                ...
                throw new Exception('GitHub OAuth hatası: ' . $error);
            }
            ...
            // Email boşsa email listesinden al
            if (empty($profile['email'])) {
                $emails = $this->get_user_emails($access_token);
                if (!empty($emails) && is_array($emails)) {
                    // Önce primary email'i bul
                    foreach ($emails as $email_data) {
                        if (isset($email_data['primary']) && $email_data['primary'] && isset($email_data['email'])) {
                            $profile['email'] = $email_data['email'];
                            break;
                        }
                    }
                    // Primary yoksa ilk email'i al
                    if (empty($profile['email']) && isset($emails[0]['email'])) {
            ...
            error_log('GitHub callback hatası: ' . $e->getMessage());
    ...
    public function find_or_create_user($profile) {
        ...
        $email = !empty($profile['email']) ? $profile['email'] : $profile['provider_slug'] . '_' . $profile['user_id'] . '@github.local';
        ...
        // GitHub spesifik bilgileri kaydet
        $github_fields = ['bio', 'company', 'location', 'blog', 'hireable', 'public_repos', 'public_gists', 'followers', 'following', 'created_at', 'updated_at'];
        foreach ($github_fields as $field) {
            if (isset($profile[$field])) {
                update_user_meta($user_id, 'ai_community_github_' . $field, $profile[$field]);
            }
        }
        
        // Token bilgilerini kaydet
        if (!empty($profile['access_token'])) {
            update_user_meta($user_id, 'ai_community_github_access_token', $profile['access_token']);
        }
    ...
    public function get_access_token($code) {
        ...
        if (is_wp_error($response)) {
            error_log('GitHub token alma hatası: ' . $response->get_error_message());
            return $response;
        }
    ...
    public function get_userinfo($token) {
        ...
        if (is_wp_error($response)) {
            error_log('GitHub userinfo alma hatası: ' . $response->get_error_message());
            return $response;
        }

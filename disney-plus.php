<?php
/**
 * Disney+ Mock OAuth2 Provider
 * 
 * NOT: Disney+ gerçek OAuth2 API'si sunmamaktadır.
...
class AI_Community_Provider_DisneyPlus extends AI_Community_OAuth2_Provider {
    
    protected $label = 'Disney+ (Mock)';
    protected $name = 'Disney+';
    public $slug = 'disney-plus';
    protected $is_mock = true;
    
    // Mock API endpoint'leri
    protected $auth_url = 'https://mock-auth.local/disney';
    protected $token_url = 'https://mock-token.local/disney';
    protected $userinfo_url = 'https://mock-api.local/disney/user';
    ...
    public function __construct($id, $config, $core) {
        ...
        $this->core->log('disney_mock_mode', [
            'message' => 'Disney+ provider MOCK modunda çalışıyor - Gerçek API yok'
        ]);
    }
    ...
    public function handle() {
        ...
        if (isset($params['error'])) {
            ...
            $this->core->log('disney_oauth_error', [
                'error' => $error,
    ...
    protected function handle_callback($params) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_state')) {
            $this->core->log('disney_invalid_state', ['state' => $state]);
        ...
        if (is_wp_error($token_data)) {
            $this->core->log('disney_token_error', [
        ...
        if (is_wp_error($userinfo)) {
            $this->core->log('disney_userinfo_error', [
        ...
        if (!$user_id || is_wp_error($user_id)) {
            $this->core->log('disney_user_creation_error', [
        ...
        // Başarılı giriş
        $redirect_url = apply_filters('ai_community_login_redirect', home_url('/dashboard'), $user_id, 'disney-plus');
    ...
    public function get_status() {
        return [
            ...
            'warning' => 'Disney+ gerçek API sunmamaktadır. Bu provider sadece test amaçlıdır.'
        ];
    }
    ...
    public function get_userinfo($access_token) {
        // Mock kullanıcı verileri - Disney+ temalı
        $mock_users = [
            [
                'id' => 'disney_usr_001',
                ...
                'given_name' => 'Mickey',
                'family_name' => 'Mouse',
                'picture' => 'https://ui-avatars.com/api/?name=Mickey+Mouse&background=f00&color=fff&size=200',
                'subscription' => 'premium',
                'member_since' => '2019-11-12',
                'profiles' => 7,
                'locale' => 'en-US',
                'favorite_character' => 'Donald Duck'
            ],
            [
                'id' => 'disney_usr_002',
                ...
                'given_name' => 'Elsa',
                'family_name' => 'Arendelle',
                'picture' => 'https://ui-avatars.com/api/?name=Elsa&background=00f&color=fff&size=200',
                'subscription' => 'standard',
                'member_since' => '2020-03-15',
                'profiles' => 4,
                'locale' => 'en-US',
                'favorite_character' => 'Olaf'
            ],
            [
                'id' => 'disney_usr_003',
                ...
                'given_name' => 'Simba',
                'family_name' => '',
                'picture' => 'https://ui-avatars.com/api/?name=Simba&background=fc0&color=000&size=200',
                'subscription' => 'basic',
                'member_since' => '2021-07-10',
                'profiles' => 3,
                'locale' => 'en-US',
                'favorite_character' => 'Mufasa'
            ],
            [
                'id' => 'disney_usr_004',
                'email' => 'ahmet@disney.com.tr',
                'name' => 'Ahmet Yılmaz',
                'given_name' => 'Ahmet',
                'family_name' => 'Yılmaz',
                'picture' => 'https://ui-avatars.com/api/?name=Ahmet+Yilmaz&background=0a0&color=fff&size=200',
                'subscription' => 'premium',
    ...
    public function get_access_token($code) {
        return [
            'access_token' => 'mock_at_disney_' . wp_generate_password(24, false),
            'refresh_token' => 'mock_rt_disney_' . wp_generate_password(24, false),
            'expires_in' => 3600,
            'token_type' => 'Bearer',
            'scope' => $this->scope
        ];
    ...
    public function map_userinfo_to_profile($userinfo) {
        if (!is_array($userinfo) || empty($userinfo['id'])) {
            return [
                'id' => 'mock_' . uniqid(),
                'email' => 'mock_' . uniqid() . '@disney.local',
                'name' => 'Disney+ User',
                'avatar' => 'https://ui-avatars.com/api/?name=Disney+User&background=00f&color=fff&size=200'
            ];
        }
        ...
        return [
            ...
            'email_verified' => true,
            'name' => $userinfo['name'] ?? 'Disney+ User',
            'first_name' => $userinfo['given_name'] ?? '',
            'last_name' => $userinfo['family_name'] ?? '',
            'username' => explode('@', $userinfo['email'])[0] ?? 'disney_user',
            ...
            'favorite_character' => $userinfo['favorite_character'] ?? 'Mickey Mouse'
        ];
    }
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['subscription'])) {
            update_user_meta($user_id, 'disney_subscription', $profile['subscription']);
        }
        
        if (!empty($profile['favorite_character'])) {
            update_user_meta($user_id, 'disney_favorite', $profile['favorite_character']);
        }
    ...
    protected function generate_unique_username($profile) {
        ...
        if (!empty($profile['username'])) {
            ...
            $base = 'disney_' . substr($profile['id'], -8);
        } else {
            $base = 'disney_user';
        }

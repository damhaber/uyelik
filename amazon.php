<?php
/**
 * AI Community Provider: Amazon
 * Amazon Login with OAuth2
 * 
...
/**
 * Amazon OAuth Provider sınıfı
 */
class AI_Community_Provider_Amazon extends AI_Community_OAuth2_Provider {
     ...
     * @var string
     */
    protected $provider_name = 'Amazon';
    ...
    /**
     * İzin kapsamı (scope)
     * Amazon scopes: https://developer.amazon.com/docs/login-with-amazon/scope.html
     *
     * @var string
     */
     ...
     * @var string
     */
    protected $auth_url = 'https://www.amazon.com/ap/oa';
     ...
     * @var string
     */
    protected $token_url = 'https://api.amazon.com/auth/o2/token';
     ...
     * @var string
     */
    protected $userinfo_url = 'https://api.amazon.com/user/profile';
    ...
    public function __construct($id, $config = [], $core = null) {
        ...
        if (isset($config['redirect_uri'])) {
            ...
            $this->redirect_uri = home_url('/ai-community/?provider=amazon&action=callback');
        }
    ...
    public function get_auth_url($state = '') {
        ...
        if (!empty($state)) {
            ...
            $params['state'] = wp_create_nonce('amazon_oauth_' . get_current_user_id());
        }
     ...
     * Amazon API yanıtı: https://developer.amazon.com/docs/login-with-amazon/obtain-customer-profile.html
     *
    ...
    public function map_userinfo_to_profile($userinfo) {
        ...
        // ID (Amazon user_id)
        $profile['id'] = isset($userinfo['user_id']) ? (string) $userinfo['user_id'] : '';
        ...
        if (isset($userinfo['name'])) {
            ...
            $profile['name'] = 'Amazon User';
        }
        
        // Amazon'dan avatar gelmez, boş bırak
        $profile['avatar'] = '';
    ...
    protected function handle_callback($request) {
        ...
        if (!empty($state)) {
            if (!wp_verify_nonce($state, 'amazon_oauth_' . get_current_user_id())) {
                return [
                    'success' => false,
    ...
    protected function handle_auth($request) {
        $state = wp_create_nonce('amazon_oauth_' . get_current_user_id());

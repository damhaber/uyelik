...
class AI_Community_Provider_GitLab extends AI_Community_OAuth2_Provider {
    
    protected $label = 'GitLab';
    protected $name = 'GitLab';
    public $slug = 'gitlab';
    
    // API endpoint'leri
    protected $auth_url = 'https://gitlab.com/oauth/authorize';
    protected $token_url = 'https://gitlab.com/oauth/token';
    protected $userinfo_url = 'https://gitlab.com/api/v4/user';
    
    // OAuth2 ayarları
    protected $scope = 'read_user';
    ...
    public function handle() {
        ...
        if (isset($params['error'])) {
            ...
            $this->core->log('gitlab_oauth_error', [
                'error' => $error,
    ...
    protected function handle_callback($params) {
        ...
        if (!wp_verify_nonce($state, 'ai_community_oauth_state')) {
            $this->core->log('gitlab_invalid_state', ['state' => $state]);
        ...
        if (is_wp_error($token_data)) {
            $this->core->log('gitlab_token_error', [
        ...
        if (is_wp_error($userinfo)) {
            $this->core->log('gitlab_userinfo_error', [
        ...
        if (!$user_id || is_wp_error($user_id)) {
            $this->core->log('gitlab_user_creation_error', [
        ...
        // Başarılı giriş
        $redirect_url = apply_filters('ai_community_login_redirect', home_url('/dashboard'), $user_id, 'gitlab');
    ...
    public function get_userinfo($access_token) {
        ...
        if (isset($data['error'])) {
            return new WP_Error('gitlab_api_error', $data['error'] ?? 'Unknown error');
        }
    ...
    public function get_access_token($code) {
        ...
        if (isset($data['error'])) {
            return new WP_Error('gitlab_token_error', $data['error_description'] ?? $data['error'] ?? 'Unknown error');
        }
    ...
    public function map_userinfo_to_profile($userinfo) {
        ...
        $name_parts = explode(' ', $userinfo['name'] ?? '', 2);
        $first_name = $name_parts[0] ?? '';
        $last_name = $name_parts[1] ?? '';
        
        return [
            'id' => (string)$userinfo['id'],
            'email' => $userinfo['email'] ?? $userinfo['public_email'] ?? '',
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['company'])) {
            update_user_meta($user_id, 'company', $profile['company']);
        }
    ...
    protected function generate_unique_username($profile) {
        ...
        if (!empty($profile['username'])) {
            ...
            $base = 'gitlab_' . substr($profile['id'], 0, 8);
        } else {
            $base = 'gitlab_user';
        }

...
class AI_Community_Provider_Kakao extends AI_Community_OAuth2_Provider {
    
    protected $config = [];
    protected $version = 'OAuth2';
    public $slug = 'kakao';
    protected $scope = 'profile_nickname,profile_image,account_email';
    ...
    public function parse_expires_in($response) {
        ...
        return 3600; // Varsayılan 1 saat (Kakao'da genelde 6-12 saat)
    }
    ...
    public function map_userinfo_to_profile($userinfo) {
        // Kakao response formatı: { id: 123, properties: { ... }, kakao_account: { ... } }
        $properties = isset($userinfo['properties']) ? $userinfo['properties'] : [];
        $account = isset($userinfo['kakao_account']) ? $userinfo['kakao_account'] : [];
        
        $profile = [
            'provider' => $this->get_name(),
            'provider_slug' => $this->get_slug(),
            'user_id' => isset($userinfo['id']) ? (string) $userinfo['id'] : '',
            'username' => isset($properties['nickname']) ? $properties['nickname'] : '',
            'display_name' => '',
            'first_name' => isset($properties['nickname']) ? $properties['nickname'] : '',
            'last_name' => '',
            'email' => isset($account['email']) ? $account['email'] : '',
            'avatar' => isset($properties['profile_image']) ? $properties['profile_image'] : 
                       (isset($properties['thumbnail_image']) ? $properties['thumbnail_image'] : ''),
            'profile_url' => isset($properties['profile_url']) ? $properties['profile_url'] : '',
            'locale' => '',
            'gender' => '',
            'birthday' => isset($account['birthday']) ? $account['birthday'] : '',
            'phone_number' => isset($account['phone_number']) ? $account['phone_number'] : '',
            'age_range' => isset($account['age_range']) ? $account['age_range'] : '',
            'is_email_verified' => isset($account['is_email_verified']) ? (bool)$account['is_email_verified'] : false,
            'is_email_valid' => isset($account['is_email_valid']) ? (bool)$account['is_email_valid'] : false
        ];
        
        // Display name oluştur
        if (!empty($properties['nickname'])) {
            $profile['display_name'] = $properties['nickname'];
        } elseif (!empty($account['email'])) {
            $profile['display_name'] = $account['email'];
        }
        
        // Gender çevir (Kakao'da male/female veya M/F olabilir)
    ...
    public function find_or_create_user($profile) {
        ...
        if (!empty($profile['phone_number'])) {
            update_user_meta($user_id, 'ai_community_kakao_phone', $profile['phone_number']);
        }
        
        if (!empty($profile['birthday'])) {
            update_user_meta($user_id, 'ai_community_kakao_birthday', $profile['birthday']);
        }
